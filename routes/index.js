...
// Home
    router.get('/', (req, res) => {
        User.find({user_status: 1}).then((users) => {
            Book.find({visibility_status: 1}).sort({date: 'asc'}).limit(12).then((books) => {
                Post.find({visibility_status: 1}).sort({date: 'asc'}).limit(12).then((posts) => { 
...
// Biblioteca
    router.get('/biblioteca', (req, res) => {
        User.find({user_status: 1}).then((users) => {
            Book.find({visibility_status: 1}).sort({date: 'asc'}).then((books) => {
...
// Artigos
    router.get('/artigos', (req, res) => {
        User.find({user_status: 1}).then((users) => {
            Post.find({visibility_status: 1}).sort({date: 'asc'}).then((posts) => {
...
// Página do Usuário
    router.get('/profile/:nickname', (req, res) => {
        User.findOne({nickname:req.params.nickname, access: 1, user_status: 1}).lean().then((user_detail) => {
            if(user_detail){
                Book.find({user: user_detail._id, visibility_status: 1}).sort({date: 'asc'}).then((books) => {
                    Post.find({user: user_detail._id, visibility_status: 1}).sort({date: 'asc'}).then((posts) => { 
...
// Pesquisa
    router.get('/search/', (req, res) => {
        ...
        User.find({user_status: 1}).then((users) => {
            Book.find({ $or:[ {'name':{$regex: search_terms, $options: "i"}}, {'author':{$regex: search_terms, $options: "i"}}, {'hashtags':{$regex: search_terms, $options: "i"}} ], visibility_status: 1}).sort({date: 'asc'}).then((books) => {
                Post.find({ $or:[ {'title':{$regex: search_terms, $options: "i"}}, {'hashtags':{$regex: search_terms, $options: "i"}} ], visibility_status: 1}).sort({date: 'asc'}).then((posts) => {
